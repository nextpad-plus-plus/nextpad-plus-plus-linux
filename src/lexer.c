...
static const char *ext_to_lang(const char *ext)
{
    ...
    /* P4 — consult ~/.nextpad++/langs.xml first; the hardcoded kExtLang
...
{
    /* P4 — consult ~/.nextpad++/langs.xml first for each keyword set.
     * macOS uses up to 4 keyword sets: instre1/type1/type2/type3 — push
     * each to its slot. Fall back to our hardcoded kKeywords table if
     * langs.xml doesn't define this language. */
    gboolean wrote_any = FALSE;
    static const char *macos_kw_names[] = {
        "instre1", "instre2", "type1", "type2", "type3", "type4",
        "substyle1", "substyle2", "substyle3", "substyle4",
        "substyle5", "substyle6", "substyle7", "substyle8",
        NULL
    };
    for (int slot = 0; slot < 9 && macos_kw_names[slot]; slot++) {
        const char *kw = langsmgr_keywords(lang, macos_kw_names[slot]);
        if (kw && *kw) {
            sci_msg(sci, SCI_SETKEYWORDS, slot, (sptr_t)kw);
            wrote_any = TRUE;
        }
    }
    if (!wrote_any) {
