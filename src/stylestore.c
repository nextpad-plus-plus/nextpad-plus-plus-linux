...
void stylestore_apply_global(GtkWidget *sci)
{
    ...
    const StyleEntry *fold_active = find_global("Fold active");
    int fold_fg     = (fold && fold->fg >= 0) ? fold->fg : 0x808080;
    int fold_bg     = (fold && fold->bg >= 0) ? fold->bg : 0xF3F3F3;
    /* SCI_MARKERSETBACKSELECTED highlights the fold block under the
     * caret. macOS defaults to red (0x0000CC BGR == #CC0000) when no
     * "Fold active" entry is set. */
    int fold_active_fg = (fold_active && fold_active->fg >= 0)
                         ? fold_active->fg : 0x0000CC;
    /* All 7 fold markers: FOLDEREND .. FOLDEROPEN (25..31) — covers the
     * box/circle/arrow glyphs plus the connector lines (FOLDERSUB,
     * TCORNER, LCORNER). Mirrors macOS EditorView.mm:1555-1569 which
     * iterates the same range and calls FORE/BACK/BACKSELECTED for each. */
    for (int mn = SC_MARKNUM_FOLDEREND; mn <= SC_MARKNUM_FOLDEROPEN; mn++) {
        ...
        sci_msg(sci, SCI_MARKERSETBACKSELECTED, (uptr_t)mn, fold_active_fg);
    }
