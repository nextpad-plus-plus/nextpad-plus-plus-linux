...
void UTF8DocumentIterator::readCharacter()
{
	...
	if (currentChar & 0x80)
	{
		...
		// macOS/clang wchar_t is 32-bit, so we emit one UTF-32 code point per
		// character — no surrogate pairs (the Windows build, with 16-bit wchar_t,
		// split astral code points across m_character[0]/[1]). m_utf16Length stays
		// 1 here, so the surrogate-stepping branches in operator++/-- are never
		// taken; boost::wregex then matches on whole Unicode code points.
		m_utf16Length = 1;
